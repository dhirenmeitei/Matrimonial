        ...
        <div class="card mb-4 shadow-sm rounded-3">
            ...
            <div class="card-header d-flex justify-content-between align-items-center bg-white border-0">
                <div class="d-flex gap-3 align-items-center">
                    <img src="{{ $post->user->photo ? asset('storage/' . $post->user->photo) : asset('img/default-user.png') }}"
                         class="rounded-circle" width="45" height="45" style="object-fit:cover">
                    <div>
                        <a href="{{ route('profile.viewprofile', $post->user->id) }}" class="fw-semibold text-dark text-decoration-none">
                            {{ $post->user->username }}
                        </a>
            ...
            <div class="card-body pb-2">
                ...
                <div class="d-flex align-items-center gap-3 mb-2">
                    <i class="bi {{ $liked ? 'bi-heart-fill text-danger' : 'bi-heart' }} fs-5 like-icon" data-post-id="{{ $post->id }}" style="cursor:pointer;"></i>
                ...
                {{-- COMMENT FORM --}}
                <form method="POST" action="{{ route('post.comment', $post->id) }}" class="d-flex gap-2">
                    @csrf
                    <input type="text" name="comment" class="form-control form-control-sm rounded-pill" placeholder="Write a comment…">
